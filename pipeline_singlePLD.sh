...
CmdName=$(basename "$0")
Syntax="${CmdName} [-c config][-a ASLZip][-m M0Zip][-s SubjectID][-j JSON][-v][-n][-l]"
function sys {
...
# If the data is GE, we need to skip some processing
if [ -n "$opt_g"]; then
        ge_data="$opt_g"
else
        ge_data=$( jq '.config.ge' "$ConfigJsonFile" | tr -d '"' )
fi
...
if [ $ge_data == TRUE ]; then
    ## add GE processing here
    echo "GE processing"
else
    ...
    # Exception so that any type of file can be taken in
    dcm_file=$(find "${m0_dcmdir}" -type f ! -name "*.nii" ! -name "*.nii.gz" ! -name "*.json" | head -n 1)
    ...
    # Check optional variables in configs and convert "null" to empty string
    config_ld=$( jq -r '.config.ld // empty' "$ConfigJsonFile" )
    config_pld=$( jq -r '.config.pld // empty' "$ConfigJsonFile" )
    config_nbs=$( jq -r '.config.nbs // empty' "$ConfigJsonFile" )
    config_m0scale=$( jq -r '.config.m0_scale // empty' "$ConfigJsonFile" )

    if [ -n "$input_json" ]; then
        ## Parse JSON file
        echo "Parsing JSON file for parameters."
        ld=$(jq -r '.ld // empty' "$input_json")
        echo "ld: ${ld}"
        pld=$(jq -r '.pld // empty' "$input_json")
        echo "pld: ${pld}"
        nbs=$(jq -r '.nbs // empty' "$input_json")
        echo "nbs: ${nbs}"
        m0_scale=$(jq -r '.m0_scale // empty' "$input_json")
        echo "m0_scale: ${m0_scale}"
    elif [ -n "$config_ld" ] && [ -n "$config_pld" ] && [ -n "$config_nbs" ] && [ -n "$config_m0scale" ]; then
        echo "Parsing Flywheel input for parameters."
        ld=${config_ld}
        echo "ld: ${ld}"
        pld=${config_pld}
        echo "pld: ${pld}"
        nbs=${config_nbs}
        echo "nbs: ${nbs}"
        m0_scale=${config_m0scale}
        echo "m0_scale: ${m0_scale}"
    else
        echo "Extracting parameters from DICOM files."
        echo $dcm_file
